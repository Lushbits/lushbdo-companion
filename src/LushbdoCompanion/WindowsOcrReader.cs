...
public sealed class WindowsOcrReader : IOcrReader
{
    ...
    private SoftwareBitmap? _input;    // reallocated only when the window's *size* changes
    ...
    public async Task<List<OcrRows.Piece>> ReadAsync(byte[] bgra, int width, int height, int top, int bottom)
    {
        ...
        foreach (var line in result.Lines)
        {
            ...
            pieces.Add(new OcrRows.Piece(x / _scale, lineTop / _scale + top, (lineBottom - lineTop) / _scale, text));
        }
        return pieces;
    }

    private unsafe void Fill(byte[] source, int width, int top, int bottom)
    {
        // Only the size matters to the buffer; a window that slides keeps it.
        // The watcher quantizes strip heights so this settles on a handful.
        var rows = bottom - top;
        if (_input is null || _inputRows != rows)
        {
            _input?.Dispose();
            _input = new SoftwareBitmap(BitmapPixelFormat.Bgra8, width * _scale, rows * _scale, BitmapAlphaMode.Ignore);
            _inputRows = rows;
        }
        ...
        fixed (byte* src = source)
        {
            ...
            for (var y = 0; y < rows * _scale; y++)
            {
                var srcRow = (uint*)(src + (top + y / _scale) * srcStride);
